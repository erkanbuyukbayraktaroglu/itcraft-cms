<form method="post" action="{{ $action }}">
    ...
    <div class="card" style="margin-bottom:22px;">
        <h2 style="margin-top:0;">Yazı Bilgileri</h2>
...
slug ?? '') }}"
        ...
        <div class="form-grid form-grid-3">
            <div>
                ...
                @php $selectedCategory = old('post_category_id', $post->post_category_id ?? null); @endphp
        ...
        <textarea class="form-control" id="summary" name="summary" rows="3">{{ old('summary', $post->summary ?? '') }}</textarea>

        <label for="content">İçerik</label>
        <textarea class="form-control editor-area rich-editor" id="content" name="content" rows="16">{{ old('content', $post->content ?? '') }}</textarea>
        ...
        <div class="form-grid form-grid-2">
            <div>
                ...
                <input class="form-control" type="text" id="image" name="image"
                       ...
                       placeholder="uploads/blog/ornek.jpg veya tam URL">
            </div>
    ...
    <div class="card" style="margin-bottom:22px;">
        ...
        <div class="form-grid form-grid-2">
            <div>
                ...
                @php $robotsIndex = old('robots_index', $post->robots_index ?? 'index'); @endphp
            ...
            <div>
                ...
                @php $robotsFollow = old('robots_follow', $post->robots_follow ?? 'follow'); @endphp
        ...
        <textarea class="form-control" id="og_description" name="og_description" rows="3">{{ old('og_description', $post->og_description ?? '') }}</textarea>
        ...
        <select class="form-control" id="schema_type" name="schema_type">
            @php $schemaValue = old('schema_type', $post->schema_type ?? 'Article'); @endphp
            @foreach(['Article', 'BlogPosting', 'NewsArticle', 'WebPage', 'Organization'] as $schema)
